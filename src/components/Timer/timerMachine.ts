...
export type TimerEvent = 
  ...
  export function deriveRemainingSec(state: TimerState, nowMs: number): number{
    ...
    const extraPausedMs = state.status === "PAUSED" && state.pausedAtMs != null ? Math.max(0, nowMs - state.pausedAtMs) : 0;

    const elapsedMs = nowMs - state.startedAtMs - state.accumPausedMs - extraPausedMs;
  ...
  export function reduceTimer(state: TimerState, event: TimerEvent): TimerState{
    switch(event.type){
        ...
        case "SET_LABEL": return { ...state, selectedLabelId: event.labelId };
        case "SET_PREFS": return { ...state, prefs: { ...state.prefs, ...event.prefs } };
